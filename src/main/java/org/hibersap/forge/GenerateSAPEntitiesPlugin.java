...
public final class GenerateSAPEntitiesPlugin implements Plugin {
	...
	public void listProperties() {
		final Set<Entry<Object, Object>> properties = this.sapConnectionPropertiesManager.getAllSAPProperties();
	...
	public void generateSAPEntities(
		...
		final String functionName = this.shell
	 ...
	 * @throws FileNotFoundException
	 */
	private void handleConfiguration(final SessionManagerConfig sessionManagerConfig)
			throws JAXBException, SessionManagerDuplicateException, ClassNotFoundException, FileNotFoundException {
		...
		if (replace && sessionManagerNameChoice.equals(newSessionManager)) {
			...
			final String adapter = this.shell.promptRegex(
			...
			if (adapter.matches("[jJ][cC][aA]")) {
				...
				sessionManagerConfig.setJcaConnectionFactory(
						this.sapConnectionPropertiesManager.getSAPProperty("jca.connection.factory"));
				sessionManagerConfig.setJcaConnectionSpecFactory(
						this.sapConnectionPropertiesManager.getSAPProperty("jca.connectionspec.factory"));
	...
	private void addDependency(final DependencyFacet dependencyFacet, final Dependency dependency) {
		if (!dependencyFacet.hasDirectDependency(dependency)) {
			if (dependency.getArtifactId().equals("validation-api")) {
				...
				final boolean useBeanValidation = this.shell
						.promptBoolean("Do you want to  use bean validation in your project?", false);
			...
			if (versions.isEmpty()) {
				String msg = format("Dependency not found [%s:%s:%s]", dependency.getGroupId(),
